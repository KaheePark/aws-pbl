...
# @app.route('/board')
# def board_list():
#     db = pymysql.connect(host='database-1.cltmc8lqcf9q.ap-northeast-2.rds.amazonaws.com', port=3306, db='pbldb', passwd=db_password, user='admin')
        ...
        if error is None:
            ...
            if file:
                ...
                cursor.execute(
                    ...
                    (board_id, file.filename, 's3-gaheeboard-files', 'images/' + file.filename, file.content_type)
                )
            db.commit()
            cursor.close()
            db.close()
...
def download():
    ...
    if result:
        ...
        print("-------------------------------------------------------------------")
        return send_file(f'no_image.jpg', mimetype='image/jpg')
